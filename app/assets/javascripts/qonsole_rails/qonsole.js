...
modulejs.define(
  'qonsole',
  ['lib/lodash', 'lib/jquery', 'lib/codemirror', 'sprintf', 'lib/util', 'sparql-service-result'],
  function (_, $, CodeMirror, Sprintf, Util, SparqlServiceResult) {
    'use strict';
    ...
    var bindEvents = function () {
      ...
      // dialogue events
      $('#prefixEditor')
        .on('click', '#lookupPrefix', onLookupPrefix)
    ...
    var showGivenQuery = function (query) {
      if (query === '_localstore') {
        ...
        if (window.location.hostname === 'localhost' && !query) {
          console.error(
            "No query found in local storage due to cross-origin blocking. Please check the sessionStorage tab in your browser's dev tools for the SPARQL query value.",
          );
        }
      }
      displayQuery(query);
    ...
    var queryEditor = function () {
      if (!_queryEditor) {
        _queryEditor = new CodeMirror($('#query-edit-cm').get(0), {
          lineNumbers: true,
          mode: 'sparql',
          screenReaderLabel: 'SPARQL query editor',
        });
      }
      return _queryEditor;
    };
    ...
    var assembleCurrentPrefixes = function () {
      var l = $('ul.prefixes input[type=checkbox]:checked').map(function (i, elt) {
        return { name: $.trim($(elt).parent().text()), uri: $(elt).val() };
      });
      return $.makeArray(l);
    };
    ...
    var currentConfiguration = function () {
      ...
      _.each(prefixList, function (pair) {
        config.prefixes[pair.name] = pair.uri;
      });
    ...
    var onQueryFail = function (jqXHR) {
      var text =
        jqXHR.valueOf().responseText ||
        Sprintf.sprintf("Sorry, that didn't work because: '%s'", jqXHR.valueOf().statusText);
      renderFailure(text);
    };
    ...
    var onLookupPrefix = function (e) {
      ...
      if (prefix) {
        $.getJSON(Sprintf.sprintf('http://prefix.cc/%s.file.json', prefix), function (data) {
